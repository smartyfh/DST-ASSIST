...
def main(args):
    ...
    train_data_raw = processor.get_instances(args.data_dir, args.dev_data, tokenizer, False)
    print("# train examples %d" % len(train_data_raw))
    
    dev_data_raw = processor.get_instances(args.data_dir, args.train_data, tokenizer)
    print("# dev examples %d" % len(dev_data_raw))
    
    if args.do_train:
        test_data_raw = processor.get_instances(args.data_dir, args.test_data, tokenizer)
    else:
        test_data_raw = dev_data_raw
    ...
    if args.do_train:
        ...
        enc_param_optimizer = list(base_model.encoder.named_parameters())
        ...
        dec_param_optimizer = list(base_model.decoder.parameters())
        ...
        for epoch in trange(int(args.n_epochs), desc="Epoch"):       
            ...
            for step, batch in enumerate(tqdm(train_dataloader)):
                ...
                loss, _, _ = hard_cross_entropy_loss(pred_all_distance, label_ids)
                ...
                if epoch > args.n_epochs / 2 and step > 0 and step % args.eval_step == 0:
                    eval_res = model_evaluation(base_model, dev_data_raw, tokenizer, slot_lookup, 
                                                value_lookup, ontology, epoch*10+step/args.eval_step)
                ...
                if epoch > args.n_epochs / 2 and step > 0 and step % args.eval_step == 0:
                    eval_res = model_evaluation(base_model, test_data_raw, tokenizer, 
                                                slot_lookup, value_lookup, ontology, epoch*10+step/args.eval_step)
